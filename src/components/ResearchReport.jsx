...
export default function ResearchReport({ ticker, financials: fin, result }) {
  ...
  // Track which multiples were genuinely computed from SEC/market data vs. fell
  // back to the AI's guess — so we never present a made-up number as authoritative.
  const computedSet = { evRevenue: computedEvRevenue != null, evEbitda: computedEvEbitda != null, peRatio: computedPe != null, fcfYield: computedFcfYield != null }
  const noneComputed = !Object.values(computedSet).some(Boolean)
    ...
    <div style={s.root}>
        ...
        <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start' }}>
          ...
          {d.targetPrice && (
            <div style={{ textAlign: 'right' }}>
              ...
              {thinData && (
                <div style={{ fontFamily: C.mono, fontSize: 10, color: C.warning, marginTop: 4, maxWidth: 180 }}>
                  AI estimate — not DCF-modeled (no filings)
                </div>
              )}
            </div>
          )}
      ...
      <Panel style={{ marginBottom: 16 }}>
        ...
        {noneComputed && (
          <div style={{ fontSize: 12, color: C.muted2, marginBottom: 14, lineHeight: 1.6 }}>
            No SEC fundamentals available for {ticker.toUpperCase()}, so these are <span style={{ color: C.warning }}>AI-estimated</span> rather than computed from filings or live market data. Treat as directional only.
          </div>
        )}
        <div style={s.grid4}>
          {[
            ['EV/Revenue', fmtMultiple(multiples.evRevenue), computedSet.evRevenue],
            ['EV/EBITDA', fmtMultiple(multiples.evEbitda), computedSet.evEbitda],
            ['P/E', fmtMultiple(multiples.peRatio), computedSet.peRatio],
            ['FCF Yield', multiples.fcfYield != null ? pct(multiples.fcfYield) : 'N/A', computedSet.fcfYield],
          ].map(([label, value, computed]) => (
            <Metric key={label} label={label} value={value}
              sub={value !== 'N/A' && !computed ? 'AI-estimated' : undefined}
              color={value !== 'N/A' && !computed ? C.muted2 : undefined} />
          ))}
        </div>
      </Panel>
      ...
      <div style={{ marginTop: 28, paddingTop: 20, borderTop: `1px solid ${C.border}` }}>
        ...
        <div style={{ fontSize: 11, color: C.muted, lineHeight: 1.7, fontFamily: C.sans }}>
          ...
          <strong style={{ color: C.muted2 }}>Monte Carlo:</strong> {monteCarlo ? `${monteCarlo.runs.toLocaleString()} trials drawing WACC, growth, and terminal growth from normal distributions` : 'not run — insufficient cash-flow data to model a distribution'}.{' '}
